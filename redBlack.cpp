...
public:
    ...
    void leftRotate(Node node)
    {
        ...
        if (node.right != NULL)
        { // temp k left node null nahi tha or node.right me temp.left wala node add hua hai
            node.right.parent = node;
            node.right.isLeftChild = false;
        }
        if (node.parent == null)
        { // means node is a root node
            // means after rotation it becomes child of temp so temp is now new root
            root = temp;
            temp.parent = null; // because temp has become root
        }
        else
        {                              // agar node root nahi hai to temp ka new parent node ka parent ho jaayega
            temp.parent = node.parent; // we use parent to get upward link from downward

            if (node.isLeftChild)
            { // agar node left child hoga to temp bhi node ka prent ka left child hoga
    ...
    void rightRotate(Node node)
    {
        ...
        if (node.left != NULL)
        { // temp k right node null nahi tha or node.left me temp.right wala node add hua hai
    ...
    {
        if (node == null)
        { // we also conider null as a black node
            return 1;
        }
        ...
        if (node.black)
        {                      // check jo node hai kahi wo balck to nahi hai
